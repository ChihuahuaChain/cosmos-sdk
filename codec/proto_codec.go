...
func (pc *ProtoCodec) MarshalInterface(i gogoproto.Message) ([]byte, error) {
	...
	anyMarshal, err := types.NewAnyWithValue(i)
	if err != nil {
		return nil, err
	}
...
func (pc *ProtoCodec) UnmarshalInterface(bz []byte, ptr interface{}) error {
	anyUnmarshal := &types.Any{}
	err := pc.Unmarshal(bz, anyUnmarshal)
	if err != nil {
		return err
	}

	return pc.UnpackAny(anyUnmarshal, ptr)
}
...
func (pc *ProtoCodec) MarshalInterfaceJSON(x gogoproto.Message) ([]byte, error) {
	anyMarshal, err := types.NewAnyWithValue(x)
	if err != nil {
		return nil, err
	}
	return pc.MarshalJSON(anyMarshal)
}
